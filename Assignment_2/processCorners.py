...
def determine_chessboard_orientation(external_corners):
    ...
    if np.allclose(distances[::2], distances[0]):
        return 6, 8  # Orientation: 6x8
    elif np.allclose(distances[1::2], distances[1]):
        return 8, 6  # Orientation: 8x6
    else:
...
def manual_process(image):
	...
	objp[:, :2] = (115 * np.mgrid[0:8, 0:6]).T.reshape(-1, 2)
	...
	whole_imgpoints.append(np.reshape(interpolated_coordinates, (48, 1, 2)))
	interpolated_coordinates.clear()

	return whole_objpoints, whole_imgpoints
...
def click_event(event, x, y, flags, params):
	if event == cv.EVENT_LBUTTONDOWN:
		...
		cv.putText(params['img'], str(x) + ',' + str(y), (x, y), font, 0.1, (255, 0, 0), 2)
		...
		if len(params['stored_coordinates']) >= 4:
			...
			if rows and cols:
				...
				params['interpolated_coordinates'].append(ic)
				# print('Subpixel Corners:', interpolated_coordinates)

...
def calibrate(objpoints, imgpoints):
    ...
    if ret == True:
        ...
        # draw ground lines in azure
        img = cv.drawContours(img, [imgpts_cube[:4]], -1, (0, 255, 255), 3)
        # draw pillars in azure
        for i, j in zip(range(4), range(4, 8)):
            img = cv.line(img, tuple(imgpts_cube[i]), tuple(imgpts_cube[j]), (0, 255, 255), 3)
        # draw top layer in azure
        img = cv.drawContours(img, [imgpts_cube[4:]], -1, (0, 255, 255), 3)
